...
在 AstrBot 群聊 / 私聊中模拟"转生到任意世界"的一生。由「bot」代为叙述,纯叙事驱动。

## 字体下载

字体太大上传不了插件市场请从Releases中下载
...
模型会主动调 `rpg_*` 工具管理数值(HP/EXP/装备/技能/技能点),战斗推演完全由模型全权负责。

### 模式 C — DND 5E 跑团

模型是 DM + 数值管理者。所有不确定判定都用 `roll_dice` 掷骰(D20 检定 + DC),严格按 DND 5E 规则裁决。
...
- **选择性加载**(`lore_selective_load` 默认开,`lore_active_rounds` 默认 6):
  ...
  - 世界观:条目不多,**始终完整注入**(不做按需裁剪,避免模型不读历史就凭印象写)。
