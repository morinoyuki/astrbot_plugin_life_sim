...
class LifeSimPlugin(DiceMixin, RPGMixin, MdToImageMixin, Star):
    ...
    def _chat_card_avatar_prompt(self, event=None) -> str:
        ...
        而非「汐见小亚」),渲染时就可能匹配不上或误配。所以除了列出头像名单,
        还要明确要求 LLM **自行把对白角色名与名单逐一比对**,判断不出时主动用
        `角色名@头像名` 显式指定,而不是指望渲染端的模糊匹配兜底。
        ...
        return (
            "\n\n## 📷 角色头像名单(每次写对白前必须先核对)"
            "\n以下角色已设置头像:**每条对白的角色名都要先与这份名单比对**,确认"
            "能否对应上某个头像名,再按下述规则书写:" 
            "\n1. **完全一致**:角色名与名单中某项完全相同 → 直接用它作角色名,自动匹配头像;"
            "\n2. **有出入(简称/昵称/别称)**:对白想用的名字与头像名不完全一致,"
            "但你能从名单中判断出对应的是哪一张 → 必须用 `@` 显式指定该头像。"
            "判断依据包括:头像名是对白名的子串(如对白「小龟」↔ 名单「龟」)、"
            "对白名是头像名的省略(如「小亚」↔「汐见小亚」)、或语义上明显同一人。"
            "\n   格式 `角色名@头像名: 台词` —— 冒号前的角色名照常写你想用的称呼,"
            "`@` 后写名单里的头像名。例如名单里有「龟」而对白叫它「小龟」:"
            "\n   `小龟@龟: 主人你回来啦~` —— 气泡显示「小龟」,头像用「龟」;"
            "\n3. **自选借用**:让任意角色使用名单中某张头像(即使两者毫无关系,如"
            "由阴影化出的少女借用「汐见小亚」的头像),同样用 `角色名@头像名: 台词`,"
            "如 `阴影少女@汐见小亚: 你是谁?`;"
            "\n4. **确实无关**:对白角色名与名单所有项都对不上 → 正常写角色名即可,"
            "不要硬套 `@`,也不要改动名单以外的头像。"
            "\n注意:宁可多写 `@` 也不要让名字出入的头像匹配失败;但每个 `@` 后面的"
            "头像名必须是名单中的原文,不可自创。"
            "\n可用头像名单：" + lines + "\n"
        )
